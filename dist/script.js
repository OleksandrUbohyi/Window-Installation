...
var changeModalState = function changeModalState(state) {
  var windowForm = document.querySelectorAll('.balcon_icons_img'),
      ...
      //pseudo array, so forEach works correctly even if only one element
    ...
    elem.forEach(function (item, i) {
      item.addEventListener(event, function () {
        switch (item.nodeName) {
          case 'SPAN':
            state[prop] = i; // window type index
  ...
  bindActionToElems('change', windowType, 'type'); //change - for selects and checkboxes
...
var forms = function forms(state) {
    ...
    return regeneratorRuntime.async(function postData$(_context) {
      while (1) {
        switch (_context.prev = _context.next) {
          case 0:
            document.querySelector('.status').textContent = message.loading; //fetch API returns promise
  ...
  form.forEach(function (item) {
    item.addEventListener('submit', function (e) {
      e.preventDefault(); //create status message container

      var statusMessage = document.createElement('div'); //add styles to it

      statusMessage.classList.add('status'); //add container to the form submitted

      item.appendChild(statusMessage); //collect all form data including pictures, files etc.
...
var tabs = function tabs(headerSelector, tabSelector, contentSelector, activeClass) {
  ...
  tabsHeader.addEventListener('click', function (e) {
    var target = e.target; //regexp to remove the dot in the selector
